# This config fine-tunes a PoseC3D model on the FineGym-99 dataset.
...
# This is the single .pkl file containing all skeleton data and split info.
ann_file = '/home/ashisig/Research/Data/FineGym/pose/gym_gt_bboxes.pkl'
...
train_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        ann_file=ann_file,
        split='train',  # IMPORTANT: Use the 'train' split defined inside the pickle file.
...
val_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        ann_file=ann_file,
        split='val',  # IMPORTANT: Use the 'val' split defined inside the pickle file.
...
test_dataloader = dict(
    ...
    dataset=dict(
        type=dataset_type,
        ann_file=ann_file,
        split='val', # Test on the validation set since there's no test set.
...
model = dict(
    ...
    cls_head=dict(
        type='I3DHead',
        # IMPORTANT: Change this to the number of classes in FineGym (99).
        num_classes=99,
        in_channels=512,
        spatial_type='avg',
        dropout_ratio=0.5,
...
train_cfg = dict(
    type='EpochBasedTrainLoop', max_epochs=240, val_begin=1, val_interval=5)
...
# --- 7. Pre-trained Model ---
# Load weights from a model pre-trained on a larger skeleton dataset (NTU-60)
